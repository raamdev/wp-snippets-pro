...
<?= $Form->openTag(); ?>
    ...
    <?= $Form->openTable(
        ...
        <?= $Form->selectRow([
            ...
            'tip'   => __('This controls which built-in WordPress content filters are applied to content in a Snippet. All are suggested.<hr />Note: <code>wp-markdown-extra</code> is only possible if you have WP Markdown Extra installed.<hr /><code>jetpack-markdown</code> is only possible if you have Jetpack installed with Markdown enabled. The same is true for <code>jetpack-latex</code>.', 'wp-snippets'),
